...
def test_get_engine_singleton() -> None:
    with patch("common.db.get_settings", return_value=_mock_settings()):
        with patch("common.db.create_engine") as mock_create:
...
def test_get_engine_passes_url() -> None:
    ...
    with patch("common.db.get_settings", return_value=_mock_settings(url)):
        with patch("common.db.create_engine") as mock_create:
            mock_create.return_value = MagicMock()
...
def test_get_session_commits_on_success() -> None:
    ...
    with patch("common.db.get_engine", return_value=mock_engine):
        with patch("common.db.Session") as mock_session_cls:
