...
from training.split import Split, temporal_split
...
def test_split_no_overlap() -> None:
    df = _df([(2022, w) for w in range(1, 11)])
    sp = temporal_split(df, val_start=(2022, 7), test_start=(2022, 9))
    train_keys = set(zip(sp.train["iso_year"], sp.train["iso_week"]))
    val_keys   = set(zip(sp.val["iso_year"],   sp.val["iso_week"]))
    test_keys  = set(zip(sp.test["iso_year"],  sp.test["iso_week"]))
...
def test_kw53_boundary_in_val() -> None:
    ...
    assert list(zip(sp.train["iso_year"], sp.train["iso_week"])) == [(2020, 51), (2020, 52)]
    assert list(zip(sp.val["iso_year"],   sp.val["iso_week"]))   == [(2020, 53), (2021, 1)]
    assert list(zip(sp.test["iso_year"],  sp.test["iso_week"]))  == [(2021, 2)]


def test_kw53_in_train() -> None:
    sp = temporal_split(_df(_KW53), val_start=(2021, 1), test_start=(2021, 2))
    train_keys = list(zip(sp.train["iso_year"], sp.train["iso_week"]))
